...
public class TransactionExpenseDeleteApiTests : BudgetApiTestBase
{
    // Test 1: Delete expense should return 401 or 403 when request is sent without authentication
    [Test]
    public async Task Expense_Delete_Should_Return_401_Or_403_When_Unauthorized()
    {
        var budgetId = 1;
        var expenseId = 1;

        var response = await _request.DeleteAsync($"/api/budget/{budgetId}/expenses/{expenseId}");
    ...
    // Test 2: Delete expense should return 400 when authorized and expense does not exist
    [Test]
    public async Task Expense_Delete_Should_Return_400_When_Authorized_And_Expense_Does_Not_Exist()
    {
        var testLabel = "[[Expense Delete Test 2 - Expense Not Found]]";
    ...
    // Test 3: Delete expense should return 400 when authorized and expenseId is invalid (0)
    [Test]
    public async Task Expense_Delete_Should_Return_400_When_Authorized_And_ExpenseId_Is_Invalid()
    {
        var testLabel = "[[Expense Delete Test 3 - Invalid ExpenseId]]";
        var authorizedRequest = await CreateAuthorizedRequest(
            "TEST_USER_EMAIL",
            "TEST_USER_PASSWORD",
            testLabel
    ...
    // Test 4: Delete expense should return 404 when authorized and expenseId is not a number (route does not match)
    [Test]
    public async Task Expense_Delete_Should_Return_404_When_Authorized_And_ExpenseId_Is_Not_A_Number()
    {
        var testLabel = "[[Expense Delete Test 4 - NonNumeric ExpenseId]]";
        var authorizedRequest = await CreateAuthorizedRequest(
            "TEST_USER_EMAIL",
            "TEST_USER_PASSWORD",
            testLabel
    ...
    // Test 5: Delete expense should return 404 when authorized and budgetId is not a number (route does not match)
    [Test]
    public async Task Expense_Delete_Should_Return_404_When_Authorized_And_BudgetId_Is_Not_A_Number()
    {
        var testLabel = "[[Expense Delete Test 5 - NonNumeric BudgetId]]";
        var authorizedRequest = await CreateAuthorizedRequest(
            "TEST_USER_EMAIL",
            "TEST_USER_PASSWORD",
            testLabel
    ...
    // Test 6: Delete expense should return 200 when authorized and expense exists (requires create to return created id)
    [Test]
    public async Task Expense_Delete_Should_Return_200_When_Authorized_And_Expense_Exists()
    {
        var testLabel = "[[Expense Delete Test 6 - Authorized Happy Path]]";
        var authorizedRequest = await CreateAuthorizedRequest(
            "TEST_USER_EMAIL",
            "TEST_USER_PASSWORD",
            testLabel
        ...
        var createResponse = await authorizedRequest.PostAsync(
            ...
            new()
            {
                ...
                {
                    categoryId = 1,
                    description = $"Expense to delete {Guid.NewGuid()}",
                    paymentMethod = 0,
                    amount = 10,
                    expenseType = 0,
                    receiptImageUrl = "",
        ...
        if (string.IsNullOrWhiteSpace(createBody))
            Assert.Inconclusive("Create expense returned empty body, cannot extract created expense id to perform delete happy path.");
